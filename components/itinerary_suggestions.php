<?php
    ...
    $categories = get_travel_plan_collections();
...
<section class="container px-4 my-5 py-5">
                ...
                <nav>
                    <div class="nav nav-tabs easygo-nav-tabs-alt justify-content-center" id="nav-tab" role="tablist">
                        ...
                        <?php
                        $active = "active"; // A toggle that sets the first category as active
                            foreach ($categories as $entry) {
                                $category_name = $entry["collection_name"];
                                $collection_id = $entry["collection_id"];
                                echo "<button class='nav-link $active' id='$collection_id-tab' data-bs-toggle='tab' data-bs-target='#nav-$collection_id' type='button' role='tab' aria-controls='nav-$collection_id' aria-selected='false'>$category_name</button>";
                                $active = ""; // Disabling the active toggle
                            }
                        ?>
                ...
                <div class="tab-content" id="nav-tabContent">
                    <?php
                    $show = "show active"; // Variable used as a marker to make only the first collection visibile
                    foreach ($categories as $entry) {
                        $category_name = $entry["collection_name"];
                        $collection_id = $entry["collection_id"];

                    echo "
                    <div class='tab-pane fade $show' id='nav-$collection_id' role='tabpanel' aria-labelledby='$collection_id-tab'>
                    <div class='itinerary-cards-container easygo-scroll-bar scroll-h'>";
                    $show =""; // Making subsequent collections invisible till selected
                $itineraries = get_featured_itineraries($collection_id);
                // $itineraries = array_slice(get_featured_itineraries(),0,3);
                foreach ($itineraries as $category_itinerary) {
                    $entry = get_itinerary_by_id($category_itinerary["itinerary_id"]);
                    ...
                    for($i = 0; $i <sizeof($activities); $i++){
                        ...
                        if ($category_itinerary["media_location"]){
                            $suggested_image = $category_itinerary["media_location"];
                        }else{
